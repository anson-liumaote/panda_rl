...
def resample_data_to_higher_frequency(data: np.ndarray, original_fps: float, target_fps: float) -> np.ndarray:
    """
    Resample data from original_fps to target_fps using interpolation.
    
    Args:
        data: Original data with shape (frames, 4 legs, 3 coordinates)
        original_fps: Original frames per second
        target_fps: Target frames per second
    
    Returns:
        Resampled data with shape (new_frames, 4 legs, 3 coordinates)
    """
    original_frames = data.shape[0]
    
    # Calculate new number of frames
    new_frames = int(original_frames * (target_fps / original_fps))
    
    # Create time arrays for original and new data
    original_time = np.linspace(0, original_frames / original_fps, original_frames)
    new_time = np.linspace(0, original_frames / original_fps, new_frames)
    
    # Initialize the output array
    resampled_data = np.zeros((new_frames, 4, 3))
    
    # Resample each coordinate of each leg separately
    for leg in range(4):
        for coord in range(3):
            ...
            resampled_data[:, leg, coord] = interp_func(new_time)
    
    print(f"Resampled data from {original_frames} frames ({original_fps} Hz) to {new_frames} frames ({target_fps} Hz)")
...
def analyze_gait(endpoint_data: np.ndarray, output_path: str, pkl_output_path: str, fps: float = 100.0) -> Tuple[Dict, Dict, Dict]:
    """
    ...
    Args:
        ...
        pkl_output_path: Path to save the contact patterns as pkl
    ...
    # Create a list to store the contact patterns as strings
    contact_pattern_strings = []
    
    # Convert boolean values to pattern strings
    for t in range(contact_array.shape[1]):
        pattern = contact_array[:, t].astype(int)
        pattern_str = ''.join(map(str, pattern))
        contact_pattern_strings.append(pattern_str)
    
    # Find runs of the same pattern and compress them
    compressed_patterns = []
    if len(contact_pattern_strings) > 0:
        current_pattern = contact_pattern_strings[0]
        current_count = 1
        
        for t in range(1, len(contact_pattern_strings)):
            if contact_pattern_strings[t] == current_pattern:
                current_count += 1
            else:
                compressed_patterns.append((current_pattern, current_count))
                current_pattern = contact_pattern_strings[t]
                current_count = 1
        
        # Add the last pattern run
        compressed_patterns.append((current_pattern, current_count))
    
    # Create a text file for the contact patterns in the compressed format
    with open(output_path, 'w') as f:
        f.write("# Foot contact patterns [FL FR RL RR] with consecutive frame counts\n")
        f.write("# Format: ['pattern'] * count\n")
        for pattern, count in compressed_patterns:
            line = f"['{''.join(pattern)}'] * {count}"
            f.write(line + "\n")
            
            # Also print to console (limited to first 10 patterns to avoid flooding)
            if len(compressed_patterns) <= 10 or compressed_patterns.index((pattern, count)) < 10:
                print(line)
        
        if len(compressed_patterns) > 10:
            print(f"... (and {len(compressed_patterns) - 10} more patterns)")
        
        print(f"Saved {len(compressed_patterns)} compressed patterns to {output_path}")
    
    # Save the uncompressed contact patterns as a pkl file
    with open(pkl_output_path, 'wb') as f:
        pickle.dump(contact_pattern_strings, f)
    print(f"Saved {len(contact_pattern_strings)} contact patterns as pkl to {pkl_output_path}")
...
def main():
    ...
    base_name = os.path.splitext(os.path.basename(endpoint_file))[0]
    
    output_txt = os.path.join(output_dir, f"{base_name}_contact_patterns_dx.txt")
    output_pkl = os.path.join(output_dir, f"{base_name}_contact_patterns_dx.pkl")
    
    # Load endpoint data
    endpoint_data = load_endpoint_data(endpoint_file)
    print(f"Loaded endpoint data with shape: {endpoint_data.shape}")
    
    # Original and target frames per second
    original_fps = 50.0
    target_fps = 100.0
    
    # Resample data to higher frequency
    resampled_endpoint_data = resample_data_to_higher_frequency(
        endpoint_data, 
        original_fps=original_fps, 
        target_fps=target_fps
    )
    
    # Analyze gait with the resampled data
    contact_sequences, swing_heights, duty_factors = analyze_gait(
        resampled_endpoint_data, 
        output_txt, 
        output_pkl,
        fps=target_fps
    )
    ...
    plot_gait_diagram(resampled_endpoint_data, contact_sequences, swing_heights, duty_factors, fps=target_fps)

...
def root_height_penalty(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, target_height: float = 0.2) -> torch.Tensor:
    """Penalize deviation of root position from target height
    
    Args:
        env: The environment containing the scene and assets
        asset_cfg: Configuration of the asset to track
        target_height: Target height for the root position (default: 0.2)
        
    Returns:
        torch.Tensor: Penalty for deviation from target height
    """
    # Extract the asset from the environment
    asset: RigidObject = env.scene[asset_cfg.name]
    # Calculate the error between current z position and target height
    height_error = asset.data.root_pos_w[:, 2] - target_height
    # Return squared error as penalty
